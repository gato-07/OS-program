...
class GameState:
        ...
        for team in self.teams.values():
            ...
            for player in team.players:
                status = (
                    ...
                    else f"HP: {player.hp}/{config.PLAYER_MAX_HP}"
                )
                defending = " [DEFENDIENDO]" if player.is_defending else ""
                summary.append(f"  > {player.name.ljust(15)} [{status}]{defending}")
        return "\n".join(summary)
    ...
    def process_action(self, player, command_str):
        ...
        if action == "atacar":
            if len(parts) < 2:
                status, message = (
                    "Error",
                    "Debes especificar un objetivo. Ej: atacar <nombre>",
                )
            else:
                target_name = parts[1]
        ...
        # si defiende
            ...
            return (
                "Error",
                f"Comando '{action}' no reconocido. Usa 'atacar <nombre>' o 'estado'.",
            )
    ...
    def can_start(self):
        """
        Verifica si se cumplen las condiciones para iniciar el juego.
        NO modifica el estado, solo retorna True/False.
        """
        if self.game_started:
            return False
        if len(self.teams) < config.MIN_TEAMS:
            return False
